...
const KiroTextGenerationTestLayer = KiroTextGenerationLive.pipe(
...
function withFakeAcpAgent<A, E, R>(
  ...
  return Effect.gen(function* () {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "t3code-kiro-text-acp-"));
      ...
      Effect.ensuring(
        serverSettings
          .updateSettings({
            providers: {
              kiro: {
                binaryPath: previousSettings.providers.kiro.binaryPath,
              },
            },
          })
          .pipe(
...
it.layer(KiroTextGenerationTestLayer)("KiroTextGenerationLive", (it) => {
  it.effect("uses ACP model config options instead of raw CLI model ids", () => {
    const requestLogDir = mkdtempSync(path.join(os.tmpdir(), "t3code-kiro-text-log-"));
    ...
    return withFakeAcpAgent(
      {
        ...
        T3_ACP_PROMPT_RESPONSE_TEXT: JSON.stringify({
          ...
          body: "- verify kiro acp model config path",
        }),
      },
      Effect.gen(function* () {
        ...
        const generated = yield* textGeneration.generateCommitMessage({
          cwd: process.cwd(),
          branch: "feature/kiro-text-generation",
          stagedSummary: "M apps/server/src/git/Layers/KiroTextGeneration.ts",
          stagedPatch:
            "diff --git a/apps/server/src/git/Layers/KiroTextGeneration.ts b/apps/server/src/git/Layers/KiroTextGeneration.ts",
          modelSelection: {
            provider: "kiro",
            model: "gpt-5.4",
            options: {
        ...
        expect(generated.body).toBe("- verify kiro acp model config path");
  ...
  it.effect("accepts json objects with extra assistant text around them", () =>
    withFakeAcpAgent(
      ...
      Effect.gen(function* () {
        ...
        const generated = yield* textGeneration.generateCommitMessage({
          cwd: process.cwd(),
          branch: "feature/kiro-noisy-json",
  ...
  it.effect("generates thread titles through Kiro ACP text generation", () =>
    withFakeAcpAgent(
      {
        T3_ACP_PROMPT_RESPONSE_TEXT: JSON.stringify({
  ...
  it.effect("closes the ACP child process after text generation completes", () => {
    const exitLogDir = mkdtempSync(path.join(os.tmpdir(), "t3code-kiro-text-exit-log-"));
    ...
    return withFakeAcpAgent(
      ...
      Effect.gen(function* () {
        ...
        const generated = yield* textGeneration.generateCommitMessage({
          cwd: process.cwd(),
          branch: "feature/kiro-runtime-close",
          stagedSummary: "M apps/server/src/git/Layers/KiroTextGeneration.ts",
          stagedPatch:
            "diff --git a/apps/server/src/git/Layers/KiroTextGeneration.ts b/apps/server/src/git/Layers/KiroTextGeneration.ts",
          modelSelection: {
