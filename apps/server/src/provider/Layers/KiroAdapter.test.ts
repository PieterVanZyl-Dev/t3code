...
async function makeMockAgentWrapper(
  ...
  const dir = await mkdtemp(path.join(os.tmpdir(), "kiro-acp-mock-"));
...
async function makeProbeWrapper(
  ...
  const dir = await mkdtemp(path.join(os.tmpdir(), "kiro-acp-probe-"));
  const wrapperPath = path.join(dir, "fake-agent.sh");
  const envExports = Object.entries(extraEnv ?? {})
    .map(([key, value]) => `export ${key}=${JSON.stringify(value)}`)
    .join("\n");
...
kiroAdapterTestLayer("KiroAdapterLive", (it) => {
  it.effect("starts a session and maps mock ACP prompt flow to runtime events", () =>
    Effect.gen(function* () {
      ...
      yield* settings.updateSettings({ providers: { kiro: { binaryPath: wrapperPath } } });
      ...
      const session = yield* adapter.startSession({
        ...
        modelSelection: { provider: "kiro", model: "default" },
      });
  ...
  it.effect("closes the ACP child process when a session stops", () =>
    Effect.gen(function* () {
      ...
      const tempDir = yield* Effect.promise(() =>
        mkdtemp(path.join(os.tmpdir(), "kiro-adapter-exit-log-")),
      );
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const adapter = yield* KiroAdapter;
        const settings = yield* ServerSettingsService;
        const threadId = ThreadId.make("kiro-concurrent-start-session");
        const tempDir = yield* Effect.promise(() =>
          mkdtemp(path.join(os.tmpdir(), "kiro-adapter-concurrent-exit-log-")),
        );
        const exitLogPath = path.join(tempDir, "exit.log");
  ...
  it.effect("maps app plan mode onto the ACP plan session mode", () =>
    Effect.gen(function* () {
      ...
      const threadId = ThreadId.make("kiro-plan-mode-probe");
      const tempDir = yield* Effect.promise(() => mkdtemp(path.join(os.tmpdir(), "kiro-acp-")));
      ...
      yield* serverSettings.updateSettings({ providers: { kiro: { binaryPath: wrapperPath } } });
      ...
      yield* adapter.startSession({
        ...
        cwd: process.cwd(),
        runtimeMode: "full-access",
        modelSelection: { provider: "kiro", model: "composer-2" },
      });
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const adapter = yield* KiroAdapter;
        const serverSettings = yield* ServerSettingsService;
        const threadId = ThreadId.make("kiro-initial-config-probe");
        const tempDir = yield* Effect.promise(() => mkdtemp(path.join(os.tmpdir(), "kiro-acp-")));
        const requestLogPath = path.join(tempDir, "requests.ndjson");
        const argvLogPath = path.join(tempDir, "argv.txt");
        yield* Effect.promise(() => writeFile(requestLogPath, "", "utf8"));
        ...
        yield* serverSettings.updateSettings({
          providers: { kiro: { binaryPath: wrapperPath } },
        });
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const adapter = yield* KiroAdapter;
        const serverSettings = yield* ServerSettingsService;
        const threadId = ThreadId.make("kiro-tool-call-probe");
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const adapter = yield* KiroAdapter;
        const serverSettings = yield* ServerSettingsService;
        const threadId = ThreadId.make("kiro-full-access-auto-approve");
        const runtimeEvents: Array<ProviderRuntimeEvent> = [];
        const settledEventTypes = new Set<string>();
        const settledEventsReady = yield* Deferred.make<void>();
        const tempDir = yield* Effect.promise(() => mkdtemp(path.join(os.tmpdir(), "kiro-acp-")));
  ...
  it.effect("segments assistant messages around ACP tool activity in full-access mode", () =>
    Effect.gen(function* () {
      const adapter = yield* KiroAdapter;
      const serverSettings = yield* ServerSettingsService;
      const threadId = ThreadId.make("kiro-assistant-tool-segmentation");
      const runtimeEvents: Array<ProviderRuntimeEvent> = [];
      const settledEventTypes = new Set<string>();
      const settledEventsReady = yield* Deferred.make<void>();
  ...
  it.effect("cancels pending ACP approvals and marks the turn cancelled when interrupted", () =>
    Effect.gen(function* () {
      const adapter = yield* KiroAdapter;
      const serverSettings = yield* ServerSettingsService;
      const threadId = ThreadId.make("kiro-cancel-probe");
      const tempDir = yield* Effect.promise(() => mkdtemp(path.join(os.tmpdir(), "kiro-acp-")));
      const requestLogPath = path.join(tempDir, "requests.ndjson");
      const argvLogPath = path.join(tempDir, "argv.txt");
      yield* Effect.promise(() => writeFile(requestLogPath, "", "utf8"));
  ...
  it.effect("stopping a session settles pending approval waits", () =>
    Effect.gen(function* () {
      const adapter = yield* KiroAdapter;
      const serverSettings = yield* ServerSettingsService;
      const threadId = ThreadId.make("kiro-stop-pending-approval");
  ...
  it.effect.skip("stopping a session settles pending user-input waits", () =>
    Effect.gen(function* () {
      const adapter = yield* KiroAdapter;
      const serverSettings = yield* ServerSettingsService;
      const threadId = ThreadId.make("kiro-stop-pending-user-input");
  ...
  it.effect.skip("interrupting a session settles pending user-input waits", () =>
    Effect.gen(function* () {
      const adapter = yield* KiroAdapter;
      const serverSettings = yield* ServerSettingsService;
      const threadId = ThreadId.make("kiro-interrupt-pending-user-input");
      const userInputRequested = yield* Deferred.make<void>();
  ...
  it.effect("broadcasts runtime events to multiple stream consumers", () =>
    Effect.gen(function* () {
      const adapter = yield* KiroAdapter;
      const settings = yield* ServerSettingsService;
      const threadId = ThreadId.make("kiro-runtime-event-broadcast");
  ...
  it.effect("switches model in-session via session/set_config_option", () =>
    Effect.gen(function* () {
      ...
      yield* adapter.sendTurn({
        ...
        attachments: [],
        modelSelection: { provider: "kiro", model: "composer-2", options: { fastMode: true } },
      });
  ...
  it.effect("clears prior fast mode in-session when the next turn sets fastMode: false", () =>
    Effect.gen(function* () {
      const adapter = yield* KiroAdapter;
      const serverSettings = yield* ServerSettingsService;
      const threadId = ThreadId.make("kiro-fast-mode-reset");
      const tempDir = yield* Effect.promise(() => mkdtemp(path.join(os.tmpdir(), "kiro-acp-")));
      const requestLogPath = path.join(tempDir, "requests.ndjson");
      const argvLogPath = path.join(tempDir, "argv.txt");
      yield* Effect.promise(() => writeFile(requestLogPath, "", "utf8"));
      ...
      yield* adapter.sendTurn({
        ...
        attachments: [],
        modelSelection: { provider: "kiro", model: "composer-2", options: { fastMode: false } },
      });
