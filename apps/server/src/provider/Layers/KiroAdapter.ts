 ...
 *
 * Kiro speaks the same Agent Client Protocol as Cursor, Grok, and OpenCode, so
 * this adapter is built on the shared ACP scaffolding
 * (`AcpSessionRuntime`, `AcpAdapterSupport`, `AcpCoreRuntimeEvents`). It follows
 * the GrokAdapter shape closely: available models come from the `session/new`
 * response `.models` field and switching uses the standard ACP
 * `session/set_model`. Unlike Grok it advertises no ACP auth methods (Kiro
 * authenticates externally via AWS IAM Identity Center) and has no
...
/**
 * Opaque continuation cursor persisted so a session can be resumed via ACP
 * `session/load`: `{ version: 1, sessionId?: string }`.
 */
...
function completedStopReasonFromPromptResponse(
  ...
  // Kiro speaks standard ACP, so a settled prompt always carries its stop
  // reason. A missing response means the turn was settled from elsewhere
  // (cancel/steer) and carries no reason.
...
export function makeKiroAdapter(kiroSettings: KiroSettings, options?: KiroAdapterLiveOptions) {
  return Effect.gen(function* () {
    ...
    // Kiro speaks standard ACP and exposes no structured user-input extension
    // (Grok's `_x.ai/ask_user_question` / Cursor's `cursor/ask_question`), so
    // no user-input request is ever opened; any response is unexpected.
    const respondToUserInput: KiroAdapterShape["respondToUserInput"] = (threadId, requestId) =>
      Effect.gen(function* () {
