...
The adapter must emit the complete canonical vocabulary (verify against `CodexAdapter`
...
- **`turn.aborted`** — on `session/cancel`
- **`turn.diff.updated`** — ACP `Diff` tool-call content (must not be buried in `item.completed`)
...
| 8. Verification & docs   | Event-vocabulary audit vs Codex, full `typecheck`/`oxlint`/`test` runs, provider docs            | `docs/providers/kiro.md`, this plan                                                                                                                           |

Phases 1–3 ship together (work package A), 4–6 together (work package B), 7 (C), 8 (D).

## Definition of done

- `bun run typecheck` (`vp run -r typecheck`), `vp lint`, and `bun run test` green on
  Node 24.
- Kiro selectable in the provider picker; status shows "Kiro" + authenticated via
  `whoami`.
